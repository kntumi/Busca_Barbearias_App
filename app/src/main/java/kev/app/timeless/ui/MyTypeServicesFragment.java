...
import kev.app.timeless.databinding.MyTypeServicesBinding;
...
public class MyTypeServicesFragment extends Fragment implements EventListener<QuerySnapshot>, View.OnClickListener  {
    private MyTypeServicesBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater, R.layout.my_type_services, container, false);
    ...
    private void observarParent(String requestKey, Bundle result) {
        ...
        switch (getChildFragmentManager().getFragments().get(getChildFragmentManager().getFragments().size() - 1).getClass().getSimpleName()) {
            ...
            case "TypeServicesListFragment": if (tiposServiços.getValue() == null  || tiposServiços.getValue().size() == 0) {
                ...
                break;
            case "InsertTypeServiceFragment":
                break;
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.retryBtn:
                break;
            case View.NO_ID: requireParentFragment().getChildFragmentManager().beginTransaction().replace(R.id.layoutPrincipal, new MyServicesFragment()).commit();
                break;
        }
    }
}